...
def handle_transaction_error(error: Exception, context: str, status_code: int = 500) -> Tuple[Dict[str, str], int]:
    ...
    elif isinstance(error, InvalidTransactionStateError):
    ...
    try:
        with get_db_session() as session:
            ...
            if transaction.status != 'OPEN':
                raise InvalidTransactionStateError(f"Cannot extend due date for transaction in {transaction.status} state")
            
            # Business logic: Extension validation
            if transaction.is_extended:
                raise InvalidTransactionStateError("Book has already been extended once")
            
            current_date = utc_date_only()
            if is_date_past_due(transaction.book_due_date, current_date):
                raise InvalidTransactionStateError("Cannot extend book after due date has passed")
    ...
    try:
        with get_db_session() as session:
            ...
            if not validate_status_transition(transaction.status, 'APPROVED'):
                raise InvalidTransactionStateError(f"Cannot approve transaction in {transaction.status} state")
    ...
    try:
        with get_db_session() as session:
            ...
            if not validate_status_transition(transaction.status, 'OPEN'):
                raise InvalidTransactionStateError(f"Cannot issue book for transaction in {transaction.status} state")
            ...
            if not book_copy or book_copy.status not in ['ISSUED', 'AVAILABLE']:
                raise InvalidTransactionStateError("Book copy is not available for issue")
    ...
    try:
        with get_db_session() as session:
            ...
            if not validate_status_transition(transaction.status, 'CLOSED'):
                raise InvalidTransactionStateError(f"Cannot return book for transaction in {transaction.status} state")
    ...
    try:
        with get_db_session() as session:
            ...
            if not validate_status_transition(transaction.status, 'REJECTED'):
                raise InvalidTransactionStateError(f"Cannot reject transaction in {transaction.status} state")
    ...
    try:
        with get_db_session() as session:
            ...
            if not validate_status_transition(transaction.status, 'ESCALATED'):
                raise InvalidTransactionStateError(f"Cannot escalate transaction in {transaction.status} state")
            
            if transaction.fine_incurred <= 0:
                raise InvalidTransactionStateError("No fine to forgive for this transaction")
            
            if transaction.fine_forgiven:
                raise InvalidTransactionStateError("Fine has already been forgiven for this transaction")
...
def forgive_fine(ticket_id: str):
    ...
    try:
        with get_db_session() as session:
            ...
            # Validate status transition
            if not validate_status_transition(transaction.status, 'CLOSED'):
                raise InvalidTransactionStateError(f"Cannot forgive fine for transaction in {transaction.status} state")
