...
from constants.constants import STATUS_CODE_CATEGORY, APP_LOG_FILE
...
class StatusCode(Base):
    ...
    def delete_status_code(session: Session, code: str) -> None:
        ...
        status_code = session.execute(stmt).scalar_one_or_none()
        if not status_code:
